...
// This script teleports BTC from the C-Chain to native tokens on an Avalanche L1 blockchain
...
const MAX_UINT256 = ethers.constants.MaxUint256; // approve max BTC for testing purposes
const WAIT_TIME_MS = 60000; // Adjust the wait time as needed
const AMOUNT_TO_BRIDGE = 0.1; // Adjust the amount to bridge as needed

// Validate environment variables
if (
  !HOME_CHAIN_RPC_URL ||
  !REMOTE_CHAIN_RPC_URL ||
  !FUNDED_ADDRESS ||
  !SUBNET_BLOCKCHAIN_ID_HEX ||
  !ERC20_HOME_C_CHAIN ||
  !ERC20_HOME_TRANSFERER_C_CHAIN ||
  !NATIVE_TOKEN_REMOTE_SUBNET ||
  !L1NAME ||
  !PRIVATE_KEY
) {
  console.error(
...
const remoteProvider = new ethers.providers.JsonRpcProvider(
  REMOTE_CHAIN_RPC_URL
);

// ABI for the contract function we will call
const ERC20_HOME_TRANSFERER_C_CHAIN_ABI = [
  "function send((bytes32, address, address, address, uint256, uint256, uint256, address), uint256)",
];
...
async function sendTeleportTransaction(privateKey, amount) {
  ...
  const contract = new ethers.Contract(
    ERC20_HOME_TRANSFERER_C_CHAIN,
    ERC20_HOME_TRANSFERER_C_CHAIN_ABI,
    wallet
  );
...
async function main() {
  ...
  const amountInWei = ethers.utils.parseUnits(amount.toString(), 8);

  console.log(
    `${timestamp()} Starting teleport of ${amount} BTC to native tokens`
  );

  // Check initial balances
  const balanceBefore = await getERC20Balance(FUNDED_ADDRESS);
  console.log(
    `${timestamp()} Initial C-Chain BTC balance: ${balanceBefore} BTC`
  );

  const nativeBalanceBefore = await getNativeBalance(FUNDED_ADDRESS);
  console.log(
    `${timestamp()} Initial ${L1NAME} native balance: ${nativeBalanceBefore} tokens`
  );

  // Check for sufficient balance
  if (ethers.utils.parseUnits(balanceBefore, 8).lt(amountInWei)) {
    console.error(
      `${timestamp()} Insufficient BTC balance. Required: ${amount}, Available: ${balanceBefore}`
    );
    return;
  }

  console.log(
    `${timestamp()} Sufficient BTC balance available. Proceeding with transaction.`
  );
  ...
  if (allowance.lt(amountInWei)) {
    console.log(
      `${timestamp()} Insufficient allowance. Approving required amount...`
    );
    try {
  ...
  try {
    ...
    console.log(
      `${timestamp()} Transaction sent. Transaction Hash: ${
  ...
  console.log(`${timestamp()} Final C-Chain BTC balance: ${balanceAfter} BTC`);

  const nativeBalanceAfter = await getNativeBalance(FUNDED_ADDRESS);
  console.log(
    `${timestamp()} Final ${L1NAME} native balance: ${nativeBalanceAfter} tokens`
  );

  // Calculate gas cost
  let gasCost = ethers.BigNumber.from(0);
  if (transactionDetails.gasUsed && transactionDetails.effectiveGasPrice) {
    gasCost = transactionDetails.gasUsed.mul(
  ...
  const nativeBalanceChange =
    parseFloat(nativeBalanceAfter) - parseFloat(nativeBalanceBefore);
  const expectedNativeBalanceChange =
    parseFloat(amount) - parseFloat(gasCostEther);
  ...
  console.log(
    `${timestamp()} Amount changed in C-Chain BTC balance: ${balanceChange.toFixed(
      18
    )} tokens`
  );
  console.log(
    `${timestamp()} Amount changed in ${L1NAME} native balance: ${nativeBalanceChange.toFixed(
      18
    )} tokens`
  );
  console.log(
    `${timestamp()} Expected change in ${L1NAME} native balance: ${expectedNativeBalanceChange.toFixed(
      18
    )} tokens`
  );
  ...
  if (
    Math.abs(balanceChange + parseFloat(amount)) <
    tolerance * parseFloat(amount)
  ) {
    console.log(
      chalk.green(
        `${timestamp()} C-Chain BTC balance change successful: ${balanceChange.toFixed(
          18
        )} tokens`
      )
    );
    ...
    console.log(
      chalk.red(
        `${timestamp()} C-Chain BTC balance change failed: ${balanceChange.toFixed(
          18
        )} tokens`
      )
    );
  ...
  if (
    Math.abs(nativeBalanceChange - parseFloat(amount)) <
    tolerance * parseFloat(amount)
  ) {
    console.log(
      chalk.green(
        `${timestamp()} ${L1NAME} native balance change successful: ${nativeBalanceChange.toFixed(
          18
        )} tokens tokens (inclusive of tolerance)`
      )
    );
  } else {
    console.log(
      chalk.red(
        `${timestamp()} ${L1NAME} native balance change failed: ${nativeBalanceChange.toFixed(
          18
        )} tokens`
      )
    );
  ...
  console.log(
    `${timestamp()} Teleport complete. Transaction Hash: ${
      transactionDetails.transactionHash
    }`
  );
}
