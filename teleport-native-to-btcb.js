...
// This script teleports native tokens from on an Avalanche L1 blockchain to BTC on the C-Chain
...
async function sendTeleportTransaction(privateKey, amount) {
  ...
  const contract = new ethers.Contract(
    NATIVE_TOKEN_REMOTE_SUBNET,
    NATIVE_TOKEN_REMOTE_SUBNET_ABI,
    wallet
  );
...
async function main() {
  ...
  console.log(
    `${timestamp()} Starting teleport of ${amount} native tokens to BTC`
  );
  ...
  if (ethers.utils.parseEther(nativeBalanceBefore).lt(amountInWei)) {
    console.error(
      `${timestamp()} Insufficient balance. Required: ${amount}, Available: ${nativeBalanceBefore}`
    );
    return;
  }

  console.log(
    `${timestamp()} Sufficient balance available. Proceeding with transaction.`
  );
  ...
  try {
    ...
    console.log(
      `${timestamp()} Transaction sent. Transaction Hash: ${transactionHash}`
    );
  ...
  console.log(
    `${timestamp()} Amount changed in C-Chain BTC balance: ${balanceChange} tokens`
  );
  console.log(
    `${timestamp()} Amount changed in ${L1NAME} native balance: ${nativeBalanceChange} tokens`
  );
  ...
  if (balanceChange > 0) {
    console.log(
      chalk.green(
        `${timestamp()} Balance change successful: ${balanceChange} tokens`
      )
    );
  } else {
    console.log(
      chalk.red(`${timestamp()} Balance change failed: ${balanceChange} tokens`)
    );
  }
  ...
  console.log(
    `${timestamp()} Teleport complete. Transaction Hash: ${transactionHash}`
  );
}
